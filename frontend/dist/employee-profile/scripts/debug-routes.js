...
class RouteDebugger {
    ...
    loadRoutesFromController() {
        const controllerPath = (0, path_1.join)(__dirname, '../employee-profile.controller.ts');
        try {
            const content = (0, fs_1.readFileSync)(controllerPath, 'utf-8');
            const routePatterns = [
                {
                    method: 'POST',
    ...
    displayDTOMappings() {
        ...
        routesWithDTOs.forEach((route) => {
            ...
            try {
                const dtoPath = (0, path_1.join)(__dirname, `../dto/${this.getDTOFileName(route.dto)}`);
                ...
                const fields = Array.from(fieldMatches, (m) => m[1]).slice(0, 10);
                if (fields.length > 0) {
...
exports.RouteDebugger = RouteDebugger;
