...
export default function HROffersPage() {
  ...
  const loadData = async () => {
    try {
      ...
      // Note: In a full implementation, you would fetch offers here
      // For now, offers are tracked in offerMap when created
      // The Approve/Reject button will be available for all applications with "offer" status
    } catch (error: any) {
  ...
  const handleOpenFinalize = async (application: Application) => {
    ...
    if (!offer) {
      try {
        ...
        // If we can't fetch the offer, we'll still show the modal
        // but the user will need to provide the offer ID or we'll handle it in finalize
        ...
        if (statusCode === 404) {
          showToast("No offer found for this application. Please create an offer first.", "error");
          return;
        } else if (statusCode === 500 || !statusCode) {
          // Server error or network issue - show modal anyway with warning
          console.warn("Could not fetch offer, but proceeding with modal");
          showToast("Warning: Could not load offer details. You may need to refresh the page.", "warning");
        } else {
          showToast(errorMessage, "error");
          return;
        }
    ...
    if (offer) {
      ...
      // No offer loaded, but we'll still show the modal
      // The finalize function will need to handle this
      setSelectedOffer(null);
      setFinalizeForm({ finalStatus: OfferFinalStatus.APPROVED });
    }
  ...
  const handleFinalizeOffer = async () => {
    ...
    if (!offer || !offer._id) {
      try {
        ...
        if (statusCode === 404) {
          ...
          showToast(
            "Could not find offer. Please refresh the page or create a new offer. " +
            "Note: The backend endpoint may need to be restarted.",
            "error"
          );
        }
    ...
    if (!offer || !offer._id) {
      showToast(
        "Offer ID not found. Please refresh the page or create a new offer.",
        "error"
      );
      setIsFinalizeModalOpen(false);
      return;
