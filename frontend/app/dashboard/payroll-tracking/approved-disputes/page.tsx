...
export default function ApprovedDisputesPage() {
  ...
  const openRefundModal = (dispute: Dispute) => {
  ...
  const handleGenerateRefund = async () => {
    if (!selectedDispute || (!user?.id && !user?.userId)) {
    ...
    try {
      await payslipsApi.generateRefundForDispute(selectedDispute.disputeId, {
      ...
      // Refresh disputes list after a short delay
      setTimeout(async () => {
        try {
          const data = await payslipsApi.getApprovedDisputesForFinance();
          setDisputes(data || []);
      ...
      <div className="container mx-auto px-6 py-8">
        <div className="flex items-center justify-center min-h-[400px]">
          <div className="text-center">
            ...
            <p className="mt-4 text-gray-600">Loading approved disputes...</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="container mx-auto px-6 py-8">
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm text-gray-700">
                      ...
                      <div>
                        <span className="font-medium text-gray-500">Payroll Period:</span>{" "}
                        {dispute.payslipId?.payrollRunId?.payrollPeriod
                          ? formatDate(dispute.payslipId.payrollRunId.payrollPeriod)
                          : "N/A"}
                      </div>
                      <div>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-semibold text-gray-900 mb-2">No Approved Disputes</h3>
              <p className="text-sm text-gray-600 mb-4">
                There are no approved disputes awaiting finance adjustments.
              </p>
              <Button variant="outline" onClick={() => router.push("/dashboard/finance")}>
                Back to Finance Dashboard
              </Button>
      ...
      <Modal
        ...
        title={`Generate Refund for ${selectedDispute?.disputeId || "Dispute"}`}
        size="md"
        footer={
        ...
        {selectedDispute && (
          <div className="space-y-4">
            <div className="bg-gray-50 p-4 rounded-lg">
              <p className="text-sm text-gray-600 mb-2">Dispute Details:</p>
              <p className="text-sm font-medium text-gray-900">{selectedDispute.description}</p>
              {selectedDispute.payslipId?.payrollRunId && (
                <p className="text-xs text-gray-500 mt-1">
                  Payslip: {selectedDispute.payslipId.payrollRunId.runId}
                </p>
              )}
            </div>
