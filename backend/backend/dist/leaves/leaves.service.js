...
let LeavesService = class LeavesService {
    ...
    async approveLeaveRequest(approveDto, managerId, leaveRequestId) {
        const { status } = approveDto;
        const requestId = leaveRequestId || approveDto.leaveRequestId;
        const leaveRequestObjectId = new mongoose_2.Types.ObjectId(requestId);
        const leaveRequest = await this.leaveRequestModel
            .findById(leaveRequestObjectId)
            .exec();
        if (!leaveRequest) {
            throw new common_2.NotFoundException(`Leave request with ID ${leaveRequestObjectId} not found`);
        }
        if (leaveRequest.status !== leave_status_enum_1.LeaveStatus.PENDING) {
            throw new common_1.BadRequestException(`Leave request has already been ${leaveRequest.status}`);
        }
    ...
    async rejectLeaveRequest(rejectLeaveRequestDto, managerId, leaveRequestId) {
    ...
    async getLeaveEntitlement(employeeId, leaveTypeId) {
        const leaveEntitlement = await this.leaveEntitlementModel
            ...
            .exec();
        if (!leaveEntitlement) {
            throw new common_2.NotFoundException('Entitlement for employee ${employeeId} with leave type ${leaveTypeId} not found');
        }
    ...
    async delegateApprovalAuthority(managerId, delegateId, startDate, endDate) {
        ...
        const delegate = await this.employeeProfileModel
            ...
            .exec();
        if (!delegate) {
            throw new common_2.NotFoundException(`Employee with ID ${delegateId} not found.`);
        }
        const manager = await this.employeeProfileModel.findById(managerId).exec();
        if (!manager) {
            throw new common_2.NotFoundException(`Manager with ID ${managerId} not found.`);
        }
        if (managerId === delegateId) {
            throw new common_1.BadRequestException('Manager cannot delegate to themselves.');
        }
        if (!this.delegationMap.has(managerId)) {
            this.delegationMap.set(managerId, []);
        }
        const delegations = this.delegationMap.get(managerId);
        const hasOverlap = delegations.some((del) => del.delegateId === delegateId &&
            del.isActive &&
        ...
        if (hasOverlap) {
            throw new common_1.BadRequestException('An active delegation already exists for this delegate in the specified date range.');
        }
        const newDelegation = {
            delegateId,
            startDate,
    ...
    isDelegatedApprover(employeeId, managerId) {
        const delegations = this.delegationMap.get(managerId);
        if (!delegations || delegations.length === 0) {
            return false;
        }
        const now = new Date();
        return delegations.some((del) => del.delegateId === employeeId &&
            del.isActive &&
            now >= del.startDate &&
            now <= del.endDate);
    }
    getDelegatedManagerId(employeeId) {
        for (const [managerId, delegations] of this.delegationMap.entries()) {
            const now = new Date();
            const isActiveDelegate = delegations.some((del) => del.delegateId === employeeId &&
                del.isActive &&
                now >= del.startDate &&
                now <= del.endDate);
            if (isActiveDelegate) {
    ...
    async finalizeLeaveRequest(leaveRequestId, hrUserId) {
        const leaveRequestObjectId = new mongoose_2.Types.ObjectId(leaveRequestId);
        const leaveRequest = await this.leaveRequestModel
            .findById(leaveRequestObjectId)
            .exec();
        if (!leaveRequest) {
        ...
        if (leaveRequest.status !== leave_status_enum_1.LeaveStatus.APPROVED) {
            throw new common_1.BadRequestException(`Leave request must be APPROVED by Department Head before HR finalization. Current status: ${leaveRequest.status}`);
        }
        const lastApproval = leaveRequest.approvalFlow[leaveRequest.approvalFlow.length - 1];
        const alreadyFinalized = leaveRequest.approvalFlow.some((approval) => approval.role === 'HR Manager' &&
            approval.status === leave_status_enum_1.LeaveStatus.APPROVED);
        if (alreadyFinalized) {
            throw new common_1.BadRequestException('Leave request has already been finalized by HR Manager.');
        }
        if (!lastApproval ||
            lastApproval.role !== 'Departement_Head' ||
            lastApproval.status !== leave_status_enum_1.LeaveStatus.APPROVED) {
            throw new common_1.BadRequestException('Leave request must be approved by Department Head before HR finalization.');
        }
        if (leaveRequest.attachmentId) {
            const attachment = await this.attachmentModel
                ...
                .exec();
            if (!attachment) {
                throw new common_2.NotFoundException('Referenced attachment not found.');
            }
