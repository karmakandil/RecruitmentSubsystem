...
let RecruitmentService = class RecruitmentService {
    constructor(jobModel, applicationModel, interviewModel, offerModel, jobTemplateModel, onboardingModel, documentModel, contractModel, candidateModel, referralModel, assessmentResultModel, applicationStatusHistoryModel, employeeProfileService, organizationStructureService, payrollExecutionService, payrollConfigurationService, timeManagementService, leavesService, terminationModel, clearanceModel, employeeModel, appraisalRecordModel, employeeSystemRoleModel) {
    ...
    async previewJobRequisition(id) {
        ...
        if (!populatedJob.templateId) {
            throw new common_1.BadRequestException('Job requisition is missing template. Cannot preview without template details.');
        }
    ...
    async apply(dto, consentGiven = false) {
        ...
        const application = new this.applicationModel({
            candidateId: new mongoose_2.Types.ObjectId(dto.candidateId),
            requisitionId: new mongoose_2.Types.ObjectId(dto.requisitionId),
            assignedHr: dto.assignedHr ? new mongoose_2.Types.ObjectId(dto.assignedHr) : undefined,
...
Due: ${context.dueDate}`
    ...
    async createOnboarding(createOnboardingDto, contractSigningDate, startDate, workEmail, contractId) {
        try {
            ...
            if (contractId) {
                if (!mongoose_2.Types.ObjectId.isValid(contractId.toString())) {
                ...
                try {
                    ...
                    if (employee) {
                        const candidate = await this.candidateModel
                            .findOne({ personalEmail: employee.personalEmail })
                            .lean();
                        if (candidate) {
            ...
            if (!finalContractId) {
                throw new common_1.BadRequestException('Contract ID is required for onboarding. Please provide contractId or ensure contract exists for this employee.');
            }
    ...
    async uploadTaskDocument(onboardingId, taskIndex, file, documentType, manualDocumentData) {
        try {
            if (!mongoose_2.Types.ObjectId.isValid(onboardingId)) {
                throw new common_1.BadRequestException('Invalid onboarding ID format');
            }
            ...
            if (!hasFile && !hasManualData) {
                throw new common_1.BadRequestException('Either file upload or manual document data (nationalId/documentDescription) is required');
            }
            const onboarding = await this.onboardingModel.findById(onboardingId);
            if (!onboarding) {
                throw new common_1.NotFoundException('Onboarding not found');
            ...
            if (onboarding.completed) {
                throw new common_1.BadRequestException('Cannot upload documents for a completed onboarding checklist');
            }
            const task = onboarding.tasks[taskIndex];
            if (task.status === onboarding_task_status_enum_1.OnboardingTaskStatus.COMPLETED && task.documentId) {
            ...
            else if (onboarding.tasks[taskIndex].status === onboarding_task_status_enum_1.OnboardingTaskStatus.IN_PROGRESS) {
                onboarding.tasks[taskIndex].status = onboarding_task_status_enum_1.OnboardingTaskStatus.COMPLETED;
                onboarding.tasks[taskIndex].completedAt = new Date();
            }
            ...
            if (allCompleted) {
                ...
                onboarding.completedAt = new Date();
                try {
                    const employee = await this.employeeProfileService.findOne(onboarding.employeeId.toString());
                    if (employee && employee.personalEmail) {
                        await this.sendNotification('onboarding_completed', employee.personalEmail, {
                            employeeName: employee.firstName || 'New Hire',
                        }, { nonBlocking: true });
                    }
                }
        ...
        try {
            ...
            try {
                ...
                onboardingCreated = await this.createOnboarding({
                    employeeId: new mongoose_2.Types.ObjectId(employeeId),
                    tasks: [],
                }, contractSigningDate, startDate, workEmail, contract._id);
    ...
    async uploadContractDocument(offerId, file, documentType = document_type_enum_1.DocumentType.CONTRACT, manualDocumentData) {
        try {
            if (!mongoose_2.Types.ObjectId.isValid(offerId)) {
                throw new common_1.BadRequestException('Invalid offer ID format');
            }
            ...
            if (offer.applicantResponse !== offer_response_status_enum_1.OfferResponseStatus.ACCEPTED) {
                throw new common_1.BadRequestException('Offer must be accepted before uploading signed contract');
            }
            const candidate = await this.candidateModel
                .findById(offer.candidateId)
                .lean();
            ...
            if (!contract) {
                contract = await this.contractModel.create({
                    offerId: new mongoose_2.Types.ObjectId(offerId),
            ...
            throw new common_1.BadRequestException('Failed to upload contract document: ' + this.getErrorMessage(error));
        }
    }
    async uploadCandidateForm(offerId, file, documentType, manualDocumentData) {
        try {
            if (!mongoose_2.Types.ObjectId.isValid(offerId)) {
                throw new common_1.BadRequestException('Invalid offer ID format');
            }
            ...
            if (documentType !== document_type_enum_1.DocumentType.ID &&
                documentType !== document_type_enum_1.DocumentType.CERTIFICATE) {
                throw new common_1.BadRequestException('Invalid document type for candidate form. Must be ID or CERTIFICATE');
            }
            let filePath;
            if (hasFile) {
                const allowedTypes = [
            ...
            throw new common_1.BadRequestException('Failed to upload candidate form: ' + this.getErrorMessage(error));
        }
    }
            ...
            for (const onboarding of allOnboardings) {
                try {
                    ...
                    if (!employee.personalEmail) {
                        console.warn(`Employee ${employee._id} has no personal email for reminders`);
                        continue;
                    }
                    ...
                    const overdueTasks = onboarding.tasks.filter((task) => {
                        ...
                        if (isNaN(deadline.getTime())) {
                            console.warn(`Invalid deadline for task ${task.name} in onboarding ${onboarding._id}`);
                            return false;
                        }
                        return deadline < now;
                    }) || [];
                    ...
                    try {
                        ...
                        remindersSent++;
                        console.log(`Reminder sent to ${employee.personalEmail} for onboarding ${onboarding._id}`);
                    }
                    catch (e) {
                        remindersFailed++;
                        console.warn(`Failed to send reminder to ${employee.personalEmail}:`, this.getErrorMessage(e));
                    }
                }
                catch (error) {
                    remindersFailed++;
                    console.error(`Error processing reminder for onboarding ${onboarding._id}:`, this.getErrorMessage(error));
                }
            }
            console.log(`Onboarding reminders completed: ${remindersSent} sent, ${remindersFailed} failed`);
        }
        catch (error) {
            console.error('Error sending onboarding reminders:', error);
            throw new common_1.BadRequestException('Failed to send onboarding reminders: ' + this.getErrorMessage(error));
        }
    }
            ...
            try {
                ...
                if (activeShift && activeShift._id) {
                    const shiftAssignment = new ShiftAssignmentModel({
                        employeeId: new mongoose_2.Types.ObjectId(employeeId),
                        shiftId: new mongoose_2.Types.ObjectId(activeShift._id),
                        departmentId: employee.primaryDepartmentId
                            ? new mongoose_2.Types.ObjectId(employee.primaryDepartmentId.toString())
                            : undefined,
                        positionId: employee.primaryPositionId
                            ? new mongoose_2.Types.ObjectId(employee.primaryPositionId.toString())
                            : undefined,
                        startDate: employee.dateOfHire || employee.contractStartDate || new Date(),
                        status: index_1.ShiftAssignmentStatus.PENDING,
                    });
                    ...
                    console.log(`Clock access provisioned for employee ${employeeId} via shift assignment ${shiftAssignmentResult._id}`);
                }
                else {
                    console.warn(`No active shift found. Clock access cannot be provisioned automatically for employee ${employeeId}. Please assign a shift manually.`);
                    shiftAssignmentNote =
                        '\n[INTEGRATION] Warning: No active shift found. Clock access requires manual shift assignment.';
                }
            }
            catch (error) {
                console.warn('Failed to provision clock access via shift assignment:', this.getErrorMessage(error));
                shiftAssignmentNote =
                ...
                if (employee) {
                    ...
                    if (!existingAssignment) {
                        ...
                        if (activeShift && activeShift._id) {
                            ...
                            console.log(`Scheduled shift assignment for employee ${employeeId} starting ${startDateObj.toISOString()}`);
                        }
                        else {
                            shiftSchedulingNote =
                                '\n[INTEGRATION] Warning: No active shift found. Clock access requires manual shift assignment.';
    ...
    async triggerPayrollInitiation(employeeId, contractSigningDate, grossSalary) {
        try {
            ...
            await onboarding.save();
            console.log(`Payroll readiness confirmed for employee ${employeeId} (REQ-PY-23). Employee will be included in payroll runs automatically.`);
            return {
    ...
    async processSigningBonus(employeeId, signingBonus, contractSigningDate) {
        try {
            ...
            if (employee.primaryPositionId) {
                try {
                    const position = await this.organizationStructureService.getPositionById(employee.primaryPositionId.toString());
                    if (position && position.title) {
                        ...
                        const matchingConfig = approvedSigningBonuses.find((config) => config.positionName === position.title);
                        if (matchingConfig) {
                            ...
                            console.warn(`No signing bonus configuration found for position: ${position.title}. Signing bonus from contract will not be processed automatically.`);
                            integrationNote =
    ...
    async createTerminationRequest(dto, user) {
        ...
        if (dto.initiator === termination_initiation_enum_1.TerminationInitiation.EMPLOYEE) {
            if (!user.roles?.includes(employee_profile_enums_2.SystemRole.DEPARTMENT_EMPLOYEE)) {
        ...
        if (dto.initiator === termination_initiation_enum_1.TerminationInitiation.HR ||
            ...
            if (!user.roles?.includes(employee_profile_enums_2.SystemRole.HR_MANAGER)) {
    ...
    async updateTerminationStatus(id, dto, user) {
        if (!user || !user.roles?.includes(employee_profile_enums_2.SystemRole.HR_MANAGER)) {
        ...
        if (dto.status === termination_status_enum_1.TerminationStatus.APPROVED) {
            ...
            if (!effectiveDate || isNaN(effectiveDate.getTime())) {
                throw new common_1.BadRequestException('Termination date (effective date) is required when approving termination request');
            }
        }
            ...
            switch (dept) {
                case 'LINE_MANAGER':
                    if (departmentItem.assignedTo &&
                        ...
                        return true;
                    return (roles.includes(employee_profile_enums_2.SystemRole.DEPARTMENT_HEAD) ||
                        roles.includes(employee_profile_enums_2.SystemRole.HR_MANAGER));
                case 'IT':
                    return (roles.includes(employee_profile_enums_2.SystemRole.SYSTEM_ADMIN) || roles.includes(employee_profile_enums_2.SystemRole.HR_MANAGER));
                case 'FINANCE':
                    return (roles.includes(employee_profile_enums_2.SystemRole.FINANCE_STAFF) ||
                        roles.includes(employee_profile_enums_2.SystemRole.PAYROLL_MANAGER) ||
                        roles.includes(employee_profile_enums_2.SystemRole.PAYROLL_SPECIALIST) ||
                        roles.includes(employee_profile_enums_2.SystemRole.HR_MANAGER));
                case 'FACILITIES':
                    return (roles.includes(employee_profile_enums_2.SystemRole.HR_ADMIN) ||
                        roles.includes(employee_profile_enums_2.SystemRole.SYSTEM_ADMIN) ||
                        roles.includes(employee_profile_enums_2.SystemRole.HR_MANAGER));
                case 'ADMIN':
                    return (roles.includes(employee_profile_enums_2.SystemRole.HR_ADMIN) ||
                        roles.includes(employee_profile_enums_2.SystemRole.HR_MANAGER) ||
                        roles.includes(employee_profile_enums_2.SystemRole.SYSTEM_ADMIN));
                case 'HR':
                    return (roles.includes(employee_profile_enums_2.SystemRole.HR_EMPLOYEE) ||
                        roles.includes(employee_profile_enums_2.SystemRole.HR_MANAGER) ||
                        roles.includes(employee_profile_enums_2.SystemRole.SYSTEM_ADMIN));
                default:
                    return (roles.includes(employee_profile_enums_2.SystemRole.HR_MANAGER) || roles.includes(employee_profile_enums_2.SystemRole.SYSTEM_ADMIN));
            }
        })();
        if (!hasPermission) {
    ...
    async triggerFinalSettlement(employeeId, terminationId) {
        ...
        try {
            const leaveBalances = await this.leavesService.getEmployeeLeaveBalance(employee._id.toString());
        ...
        try {
            ...
            const processedBenefits = await this.payrollExecutionService.processTerminationResignationBenefits(systemUserId);
            const employeeBenefits = processedBenefits.filter((benefit) => benefit.employeeId?.toString() === employeeId &&
            ...
            if (employeeBenefits.length > 0) {
                settlementData.components.benefitsTermination = {
                    ...
                    benefitRecords: employeeBenefits.map((b) => ({
                ...
                try {
                    const EmployeeTerminationResignationModel = this.terminationModel.db.model('EmployeeTerminationResignation');
                    ...
                    if (existingBenefitsCheck && existingBenefitsCheck.length > 0) {
                        settlementData.components.benefitsTermination = {
                            ...
                            benefitRecords: existingBenefitsCheck.map((b) => ({
            ...
            console.warn('triggerFinalSettlement: Failed to process termination benefits:', this.getErrorMessage(err) || err);
            settlementData.errors.push({
    ...
    async revokeSystemAccess(dto, user) {
        if (!user || !user.roles?.includes(employee_profile_enums_2.SystemRole.SYSTEM_ADMIN)) {
...
exports.RecruitmentService = RecruitmentService = __decorate([
    ...
    __param(18, (0, mongoose_1.InjectModel)(termination_request_schema_1.TerminationRequest.name)),
    __param(19, (0, mongoose_1.InjectModel)(clearance_checklist_schema_1.ClearanceChecklist.name)),
    __param(20, (0, mongoose_1.InjectModel)(employee_profile_schema_1.EmployeeProfile.name)),
    __param(21, (0, mongoose_1.InjectModel)(appraisal_record_schema_1.AppraisalRecord.name)),
    __param(22, (0, mongoose_1.InjectModel)(employee_system_role_schema_1.EmployeeSystemRole.name)),
