  ...
  async scheduleInterview(dto: ScheduleInterviewDto) {
    ...
    try {
      ...
      if (dto.panel && dto.panel.length > 0) {
        ...
        // COMMENTED OUT: EmployeeProfileService integration disabled
...
Due: ${context.dueDate}`
  ...
  async createOnboarding(
    ...
    try {
      ...
      if (tasks.length === 0) {
        ...
        // COMMENTED OUT: Payroll integration not available
        // tasks.push({
  ...
  async updateOnboardingTask(
    ...
    try {
      ...
      if (!onboarding.tasks || onboarding.tasks.length === 0) {
        throw new BadRequestException('Onboarding has no tasks');
      }
  ...
  async createEmployeeFromContract(
    ...
    try {
      ...
      // COMMENTED OUT: OrganizationStructureService integration disabled
      ...
      // NOTE: Employee profile creation is disabled - returning mock employee object
      const employee = {
  ...
  async provisionSystemAccess(
    ...
    try {
      ...
      // COMMENTED OUT: TimeManagementService and EmployeeProfileService integrations disabled
  ...
  async scheduleAccessProvisioning(
    ...
    try {
      ...
      // COMMENTED OUT: EmployeeProfileService and TimeManagementService integrations disabled
  ...
  async createTerminationRequest(dto: CreateTerminationRequestDto, user: any) {
    ...
    if (
      ...
      // COMMENTED OUT: Performance Management (AppraisalRecord) integration disabled
      ...
      // COMMENTED OUT: Performance Management integration disabled
      ...
      const termination = await this.terminationModel.create({
        ...
        reason:
          dto.reason ||
          'Termination request created (performance check disabled)', // COMMENTED OUT: latestRecord.totalScore reference removed
        employeeComments: dto.employeeComments,
        terminationDate: dto.terminationDate
          ? new Date(dto.terminationDate)
          : undefined,
  ...
  async updateTerminationStatus(
    ...
    // BR: Employee separation needs an effective date - validate termination date is set before approval
    if (dto.status === TerminationStatus.APPROVED) {
      const finalTerminationDate = termination.terminationDate || (dto.terminationDate ? new Date(dto.terminationDate) : null);
      if (!finalTerminationDate) {
        throw new BadRequestException(
          'Termination date (effective date) is required before approval. Employee separation must have an effective date.',
        );
      }
  ...
  private async _findDepartmentManagerForEmployee(employee: any) {
    ...
    //   return null;
    // }
    return null; // Integration disabled - return null for department manager lookup
  }
   ...
   * 2. Final pay calculation (outstanding salary, deductions, severance) - COMMENTED OUT (payroll incomplete)
   * 3. Benefits plan termination - COMMENTED OUT (via payroll service, payroll incomplete)
   * 4. Final payroll queue - COMMENTED OUT (payroll incomplete)
   *
   * INTEGRATIONS:
   * - LeavesService: ✅ ACTIVE - For leave balance calculation
   * - PayrollExecutionService: ⏳ COMMENTED OUT - For final pay calculation, benefits termination, and processing
   *   - NOTE: All payroll integrations are commented out due to incomplete payroll subsystem
   *   - Benefits termination fills EmployeeTerminationResignation collection (processTerminationResignationBenefits)
   *   - Uncomment when PayrollExecutionService is ready
   */
  async triggerFinalSettlement(employeeId: string, terminationId: string) {
    ...
    // COMMENTED OUT: LeavesService integration disabled
    // ============================================================================
    ...
    //   // getEmployeeLeaveBalance returns an array of entitlements or a single entitlement if leaveTypeId is provided
      ...
      //       // NOTE: Salary information would be retrieved from payroll system, but payroll subsystem is incomplete
      //       // Leave encashment amount calculation is deferred to payroll service when available
      //       let encashmentAmount = null;
      //       // COMMENTED OUT: Payroll integration for encashment calculation
      //       // TODO: When payroll service is ready, uncomment:
      //       // const salary = await this.payrollExecutionService.getEmployeeSalary(employee._id.toString());
      //       // const dailyRate = salary / 30;
      //       // encashmentAmount = totalUnusedDays * dailyRate;

      //       settlementData.components.leaveEncashment = {
      //         totalUnusedDays: totalUnusedDays,
      //         encashmentAmount: encashmentAmount, // Will be calculated by payroll service when available
      //         leaveDetails: leaveDetails,
      //         calculatedAt: new Date().toISOString(),
      //         note: 'Leave balance calculated. Encashment amount calculation deferred to payroll service (payroll subsystem incomplete).',
      //       };
      //     }
      //   }
    // } catch (err) {
    ...
    // ============================================================================
    // STEP 2: FINAL PAY CALCULATION (PAYROLL EXECUTION SERVICE INTEGRATION - COMMENTED OUT)
    // ============================================================================
    // NOTE: ALL PAYROLL INTEGRATIONS COMMENTED OUT DUE TO INCOMPLETE PAYROLL SUBSYSTEM
    //
    // When payroll subsystem is complete, uncomment the following:
    //
    // try {
    ...
    // ============================================================================
    // STEP 3: BENEFITS TERMINATION (PAYROLL EXECUTION SERVICE INTEGRATION - COMMENTED OUT)
    // ============================================================================
    // NOTE: Benefits termination is handled via Payroll Execution Service, which is incomplete.
    // The payroll-execution service has a method processTerminationResignationBenefits() that:
    // - Fills the EmployeeTerminationResignation collection (relates user to benefit)
    // - Processes approved termination/resignation benefits for the employee
    // - Creates records linking employee, benefit, and termination
    //
    // COMMENTS: ALL PAYROLL INTEGRATIONS COMMENTED OUT DUE TO INCOMPLETE PAYROLL SUBSYSTEM
    //
    // When payroll subsystem is complete, uncomment the following:
    //
    // try {
    //   // Trigger benefits termination via Payroll Execution Service
    //   // This service fills the EmployeeTerminationResignation collection that relates user to benefit
    //   // Reference: payroll-execution.service.ts -> processTerminationResignationBenefits()
    //   // This method processes all approved termination/resignation benefits for approved terminations
    //
    //   const benefitsResult = await this.payrollExecutionService.processTerminationResignationBenefits(user.id || user.userId);
    //
    //   settlementData.components.benefitsTermination = {
    //     processedBenefits: benefitsResult || [],
    //     effectiveDate: termination.terminationDate,
    //     processedAt: new Date().toISOString(),
    //     note: 'Benefits termination processed via Payroll Execution Service. EmployeeTerminationResignation records created.',
    //   };
    //
    // } catch (err) {
    //   console.warn('triggerFinalSettlement: Failed to terminate benefits via payroll service:', this.getErrorMessage(err) || err);
  ...
  async getLatestAppraisalForEmployee(employeeId: string) {
    ...
    // Integration disabled - return employee info only
    return {
      employee: {
        id: employee._id,
        employeeNumber: employee.employeeNumber,
      ...
      appraisal: null, // Appraisal integration disabled
    };
  }
