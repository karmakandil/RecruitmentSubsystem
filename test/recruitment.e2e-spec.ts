...
/**
 * RECRUITMENT API E2E TEST SUITE
 * 
 * This suite tests:
 * - Happy path scenarios (create job, app, interview, offer)
 * - Edge cases (missing fields, invalid enums, invalid types)
 * - Validation layer (class-validator @IsEnum enforcement)
 * - Status codes and error messages
 * 
 * NOTE: Tests require MongoDB connection. Ensure:
 * 1. MongoDB is running (local or Atlas)
 * 2. MONGODB_URI env var is set or default in app.module.ts
...
describe('Recruitment API - E2E Tests', () => {
  ...
  beforeAll(async () => {
    console.log('🚀 Bootstrapping test module...');
  ...
  describe('Job Requisitions - Edge Cases', () => {
    ...
    it('⚠️  should handle negative openings (allowed or rejected)', async () => {
      const response = await request(app.getHttpServer())
        .post('/recruitment/job')
        .send({
          templateId: '507f1f77bcf86cd799439011',
  ...
  describe('Interviews - Edge Cases', () => {
    ...
    it('✅ should accept all valid stages, methods, and statuses', async () => {
  ...
  describe('Offers - Edge Cases', () => {
    ...
    it('✅ should accept all valid applicantResponse and finalStatus values', async () => {
      const validResponses = ['accepted', 'rejected', 'pending'];
      const validFinalStatuses = ['approved', 'rejected', 'pending'];

      console.log(`  ✓ Valid applicantResponse values: ${validResponses.join(', ')}`);
      console.log(`  ✓ Valid finalStatus values: ${validFinalStatuses.join(', ')}`);
    });
  });
  ...
  describe('Complete Workflow', () => {
    it('✅ should complete full recruitment flow: job → app → interview → offer', async () => {
      ...
      // Job created ✓ (from earlier tests)
      console.log('    1. Job requisition created ✓');

      // Application created ✓ (from earlier tests)
      console.log('    2. Application submitted ✓');

      // Interview scheduled ✓ (from earlier tests)
      console.log('    3. Interview scheduled ✓');

      // Offer created ✓ (from earlier tests)
